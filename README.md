...
A self-hosted pipeline that turns scanned notes and PDFs into a searchable, browsable study archive. Drop a file into a folder (or upload it through the browser), and it gets OCR'd, summarized, tagged, and rendered as a clean static page automatically, no manual data entry.

Built for a homelab setup: a 24/7 server handles the always-on parts, while the heavier vision/LLM processing happens on a separate Ollama Server with a GPU, connected over the LAN.
...
└──────────────┬────────────────┘
               ...
               ▼
┌───────────────────────────────┐
│  Ollama Server (on demand)    │
│  - Ollama (vision + text LLM) │
└───────────────────────────────┘
```

1. A file lands in `data/material/<subject>/`, either dropped there directly or uploaded through the web UI.
...
- Ollama has no built-in authentication, it's intended to stay on a trusted LAN only, never exposed publicly.
